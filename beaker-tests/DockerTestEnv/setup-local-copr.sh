...
#     equals the hub string -- it does not know about aliases. The behave
#     suite (copr_behave_lib.py: dnf_copr_project()) builds that 3-part form
#     directly from the frontend URL's hostname, so without this section the
#     plugin falls through to its hardcoded `https://` default and every
#     enable/download fails with a TLS error against our http-only local
#     deployments.
